...
主密码 ──PBKDF2-SHA256(600,000 次迭代)──→ AES-256-GCM 密钥
                                              ...
                                              ├── 锁定即丢弃，永不落盘
                                              └── 条目元数据与密码独立随机 nonce + GCM 认证标签
```

- **主密码永不落盘**。localStorage 只存：盐、验证器密文、条目元数据密文、密码密文。
- **GCM 认证标签**：密文被改一字节、或主密码错一个字符都会解密失败。
- **按需解密**：解锁后只解密条目元数据；密码仅在点击「显示/复制」时解密。
- **复制后 30 秒清空剪贴板**。
...
点击「保存」后，站点名、用户名和密码都会用 AES-256-GCM 加密存入浏览器 localStorage。
...
> **QR 配对的安全性**：配对码每次生成都是新的（随机盐 + 随机 nonce），并内置 30 秒有效期。
...
- **存储**：localStorage，键名 `rsa-vault`（保险库）+ `rsa-vault-sync`（同步元数据）；站点名、用户名和密码均为密文。
