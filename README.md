...
### 密码保险库（实用工具，`docs/index.html`）

**用途**：记住一个主密码，即可安全存储与取用所有站点的强密码。无需记忆每个站点密码，
也无需把密码明文存在某处。

**安全模型**（与 1Password / Bitwarden / KeePass 同类架构）：

- 主密码用 **PBKDF2-SHA256（600,000 次迭代）** 派生出 **AES-256-GCM** 密钥。
- 派生密钥 `extractable=false` —— 不可被导出，仅驻留内存，锁定即丢弃。
- 每条站点密码用独立随机 nonce + AES-GCM 加密，密文含认证标签：密文被改一字节、或主密码错一个字符都会解密失败。
- **主密码永不落盘**。localStorage 只存：盐、PBKDF2 验证器、各条目密文。
- 按需解密：仅在点击「显示/复制」时解密单条，不批量暴露明文；复制后 30 秒清空剪贴板。
- 闲置 5 分钟自动锁定。
- 内置强密码生成器（`crypto.getRandomValues` + 拒绝采样消除模偏）。

**功能**：设置/解锁、条目增删改查、按需显示与复制密码、强密码生成、修改主密码（全量重加密）、
加密备份导出/导入、重置。全程在浏览器本地，无任何网络请求。

> 为什么从 RSA 改为对称加密？用户原始设想是「记住一个私钥加解密」。
> 但 RSA 私钥长达数百位无法人脑记忆，单用户场景下 RSA 的非对称性也无收益；
> 而「主密码 → KDF 派生对称密钥」正是真实密码管理器的做法，且更简、更快、密文更小。
> RSA 演示仍保留于 [`docs/demo.html`](docs/demo.html)。

### RSA 算法演示（`docs/demo.html`）

`rsa.py` 的纯前端复刻：原生 HTML/CSS/JS + `BigInt`，零框架、零构建、零依赖。
功能与 `rsa.py` 一一对应：密钥生成、两种模式加解密、OAEP、PSS 签名/验签。
密钥 JSON 格式与 Python 版完全一致，**网页与 Python 端互通**（已三向验证：
网页 ↔ Python ↔ `cryptography` 库）。

> 限制：演示页哈希仅支持 SHA-1/256/384/512（WebCrypto 规范不含 SHA-224）。

### 本地预览

```bash
python3 -m http.server -d docs 8000
# 浏览器打开 http://localhost:8000        （密码保险库）
# 浏览器打开 http://localhost:8000/demo.html （RSA 演示）
```
...
- 主密码遗忘 = 数据无法恢复（这是无后端、无主密码存储的设计代价）。请单独、安全地记好主密码，并定期导出备份。
- 本工具为教学性实现，未做抗侧信道/抗键盘记录防护。极高价值账号仍建议配合经审计的专业密码管理器与硬件密钥。
